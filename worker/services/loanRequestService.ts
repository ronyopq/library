...
const mapLoanRequest = (
  ...
  requesterOrganization: row.requesterOrganization ?? undefined,
  requesterDesignation: row.requesterDesignation ?? undefined,
...
const getRequestRow = async (db: DbClient, requestId: number) => {
  const rows = await db
    .select({
      ...
      requesterOrganization: loanRequests.requesterOrganization,
      requesterDesignation: loanRequests.requesterDesignation,
      requesterAddress: loanRequests.requesterAddress,
      requesterPhone: loanRequests.requesterPhone,
      requesterEmail: loanRequests.requesterEmail,
      borrowedAt: loanRequests.borrowedAt,
...
export const createPublicLoanRequest = async (
  ...
  const inserted = await db
    ...
    .values({
      ...
      requesterOrganization: payload.requesterOrganization,
      requesterDesignation: payload.requesterDesignation,
...
export const decideLoanRequest = async (
  ...
  if (decision.status === "approved") {
    const createdLoan = await createLoan(
      db,
      {
        ...
        borrowerOrganization: existing.requesterOrganization ?? undefined,
        borrowerDesignation: existing.requesterDesignation ?? undefined,
        borrowerAddress: existing.requesterAddress ?? undefined,
        borrowerPhone: existing.requesterPhone ?? undefined,
        borrowerEmail: existing.requesterEmail ?? undefined,
        borrowedAt: existing.borrowedAt ?? undefined,
  ...
  await db
    ...
    .set({
      ...
      borrowedAt: existing.borrowedAt ?? null,
